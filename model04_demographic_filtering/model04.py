...
class DemographicNet(nn.Module):
    # 매핑 정보가 들어간다.
    def __init__(self, gender_dim=2, age_dim=2, major_dim=6, grade_dim=5, num_items=500, embed_dim=8, hidden_dim=64):
        super().__init__()
    ...
    def forward(self, gender, age, major, grade, item):
        ...
        x = torch.cat([gender, age, major, grade, item], dim=-1)
        out = self.mlp(x)
        return out.squeeze()
